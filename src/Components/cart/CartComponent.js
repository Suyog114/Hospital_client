...
function CartComponent({ data }) {
  ...
  const handleMinus = async () => {
    if (count > 1) {
      ...
      await axios.patch(`${API_URL}/cart/updateCart/${data.id}`, { quantity: count - 1, total_price: (count - 1) * data.product_price })
  ...
  const handleDelete = async() => {
    console.log("clicked")
    try {
      await axios.delete(`${API_URL}/cart/deleteOneCart/${data.id}`)
