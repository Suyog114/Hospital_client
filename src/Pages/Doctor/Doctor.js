...
const handleChng=(e)=>{
  axios.get(`${API_URL}/doctor/getAllDoctor?name=${e.target.value}`).then(res => setDoctorData(res.data)).catch(err => console.log(err))
}

const srchname = window.location.pathname.split('/')[2]

  useEffect(() => {
    axios.get(`${API_URL}/doctor/getAllDoctor?name=${srchname===undefined ? "" : srchname}`).then(res => setDoctorData(res.data)).catch(err => console.log(err))
  }, [])
